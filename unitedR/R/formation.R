...
validateFormation <- function(object) {
  ...
  if (any(is.na(SW))) {
    ...
    if (GK[1] > 10) {
      msg <- paste("First argument of GK should be smaller than 10. GK has strength ", 
                   GK[1], ".", sep = "", collapse = ",")
      errors <- c(errors, msg)
    }
      ...
      for (param in c("GK", "SW", "DF", "MF", "ST", "hard", "homeAdv")) {
        ...
        for (param in c("GK", "SW")) {
          obj <- eval(parse(text = param))
          if (obj[1] > 10) {
            msg <- paste("First argument of", param, " should be smaller than 10.", 
  ...
  if (is.numeric(SW) && hard[2] %% 2 != 0 && length(hard) == 5) {
    msg <- "Hardness given to the sweeper must be divisble by two."
    errors <- c(errors, msg)
  }
  
  if (is.numeric(SW) && homeAdv[2] %% 2 != 0 && length(homeAdv) == 5) {
    msg <- "Home advantage given to the sweeper must be divisble by two."
    errors <- c(errors, msg)
  }
  
  if (hard[1] %% 2 != 0 && length(hard) == 5) {
    msg <- "Hardness given to the goalkeeper must be divisble by two."
    errors <- c(errors, msg)
  }
  
  if (homeAdv[1] %% 2 != 0 && length(homeAdv) == 5) {
    msg <- "Home advantage given to the goalkeeper must be divisble by two."
    errors <- c(errors, msg)
  }
...
setMethod("getLineup", 
          ...
          function(obj) {
            ...
            if (GK > 10) {
              GK <- 10
              warning("GK was reduced to the strength 10 (after adding hardness and home advantage).")
            }
            
            if (SW > 10) {
              SW <- 10
              warning("SW was reduced to the strength 10 (after adding hardness and home advantage).")
            }
